...
function sharing_email_send_post( $data ) {
	$content  = sprintf( __( '%1$s (%2$s) thinks you may be interested in the following post:', 'jetpack-sharing' ), $data['name'], $data['source'] ); // E-1
	$content .= "\n\n";
	...
	wp_mail( $data['target'], '['.__( 'Shared Post', 'jetpack-sharing' ).'] '.$data['post']->post_title, $content, $headers); // E-1
}

function sharing_add_meta_box() {
	$post_types = get_post_types( array( 'public' => true ) );
	$title = apply_filters( 'sharing_meta_box_title', __( 'Sharing', 'jetpack-sharing' ) ); // E-1
...
function sharing_meta_box_content( $post ) {
	...
	<p>
		<label for="enable_post_sharing">
			...
			<?php _e( 'Show sharing buttons.' , 'jetpack-sharing');  // E-1 ?>
		</label>
...
function sharing_plugin_settings( $links ) {
	$settings_link = '<a href="options-general.php?page=sharing.php">'.__( 'Settings', 'jetpack-sharing' ).'</a>'; // E-1
...
function sharing_add_plugin_settings($links, $file) {
	if ( $file == basename( dirname( __FILE__ ) ).'/'.basename( __FILE__ ) ) {
		$links[] = '<a href="options-general.php?page=sharing.php">' . __( 'Settings', 'jetpack-sharing' ) . '</a>'; // E-1
		$links[] = '<a href="http://support.wordpress.com/sharing/">' . __( 'Support', 'jetpack-sharing' ) . '</a>'; // E-1
	}
...
?>
<tr valign="top">
	<th scope="row"><label for="disable_css"><?php _e( 'Disable CSS and JS', 'jetpack-sharing' );  // E-1 ?></label></th>
	<td>
		<input id="disable_css" type="checkbox" name="disable_resources" <?php if ( $disable == 1 ) echo ' checked="checked"'; ?>/>  <small><em><?php _e( 'Advanced.  If this option is checked, you must include these files in your theme manually for the sharing links to work.', 'jetpack-sharing' );  // E-1 ?></em></small>
	</td>
</tr>
<?php
}
